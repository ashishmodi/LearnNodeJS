...
let ob5 = {};           let ob6 = {};
myMap2.set(ob5, 10);    myMap2.set(ob6, 20);
...
for (let key of myMap3.keys())      {   console.log(key);   }
for (let value of myMap3.values())  {   console.log(value); }
for (let entry of myMap3.entries()) {   console.log(`${entry[0]} -> ${entry[1]}`);  }
for (let [key, value] of myMap3.entries()) {   console.log(`${key} -> ${value}`);  }

let nos = [2, 4, 6, 8];
nos.forEach(arrFun);
function arrFun(ele:number, index: number, array: []) {
